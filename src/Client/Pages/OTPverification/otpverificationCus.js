...
const OTPverificationCus =()=>{
    ...
    const cusemail = localStorage.getItem('cusemail')
    const sendme = { otp: otp.join(''), cusemail };

    const handleSubmit=async e=>{
        e.preventDefault() 
        console.log(`OTP code: ${otp} `)
        try{
            await axios.post('/api/user/create/otpconform/customer',sendme)
    ...
    const resendotp = async e=>{
        try{
            await axios.post('/api/user/create/otpconform/customer/resend')
            console.log("Resent")
        }catch(error){
            console.log("Error!!",error);
            setErr(error.response?.data?.message || "An error occurred");
...
export default OTPverificationCus;